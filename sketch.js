var helicopterIMG, helicopterSprite, packageSprite,packageIMG;
var packageBody,ground
var box1,box1Body, box2, box3;
const Engine = Matter.Engine;
const World = Matter.World;
const Bodies = Matter.Bodies;
const Body = Matter.Body;

function preload()
{
	helicopterIMG=loadImage("helicopter.png")
	packageIMG=loadImage("package.png")
}

function setup() {
	createCanvas(800, 700);
	rectMode(CENTER);

	box1=createSprite(400,670,200,20);
	box1.shapeColor=color("Red");
	box2=createSprite(500,630,20,100);
	box2.shapeColor=color("Red");
	box3=createSprite(300,630,20,100);
	box3.shapeColor=color("Red");


	
	box1Body = Bodies.rectangle
	box1Body.isStatic=true;

	packageSprite=createSprite(width/2, 80, 10,10);
	packageSprite.addImage(packageIMG)
	packageSprite.scale=0.2

	helicopterSprite=createSprite(width/2, 200, 10,10);
	helicopterSprite.addImage(helicopterIMG)
	helicopterSprite.scale=0.6
    
	groundSprite=createSprite(width/2, height-35, width,10);
	groundSprite.shapeColor=color(0);


	engine = Engine.create();
	world = engine.world;

	packageBody = Bodies.circle(width/2 , 200 , 5 , {restitution:0, isStatic:true});
	World.add(world, packageBody);

	
	

	//Create a Ground
	ground = Bodies.rectangle(width/2, 650, width, 10 , {isStatic:true} );
 	World.add(world, ground);


	Engine.run(engine);
  
}


function draw() {
  rectMode(CENTER);
  background(0);


  packageSprite.depth=groundSprite.depth;
  packageSprite.depth=packageSprite.depth+1;

  packageSprite.x= packageBody.position.x 
  packageSprite.y= packageBody.position.y 

  packageSprite.depth=helicopterSprite.depth;
  helicopterSprite.depth=helicopterSprite.depth+1;

  box1.depth=groundSprite.depth;
  box1.depth=groundSprite.depth+1;

  box2.depth=groundSprite.depth;
  box2.depth=groundSprite.depth+1;

  box3.depth=groundSprite.depth;
  box3.depth=groundSprite.depth+1;

  drawSprites();
  
  
  
 
}

function keyPressed() {
 if (keyDown = DOWN_ARROW) {
    packageBody.isStatic=false;
    
  }
}



